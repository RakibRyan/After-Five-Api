SMODS.Consumable {
    key = 'mxms_cancri',
    prefix_config = {
        key = { mod = false }
    },
    set = 'Planet',
    atlas = 'Consumables',
    pos = {
        x = 7,
        y = 0
    },
    config = {
        hand_type = 'mxms_f_double_triple',
        softlock = true
    },
    otm_credits = {
        art = { "Maxiss02" },
        code = { "theAstra" }
    },
    cost = 4,
    loc_vars = function(self, info_queue, card)
        return {
            vars =
            {
                G.GAME.hands[card.ability.hand_type].level,
                localize(card.ability.hand_type, "poker_hands"),
                G.GAME.hands[card.ability.hand_type].l_mult,
                G.GAME.hands[card.ability.hand_type].l_chips,
                colours = {
                    (G.GAME.hands[card.ability.hand_type].level == 1 and G.C.UI.TEXT_DARK or G.C.HAND_LEVELS[math.min(7, G.GAME.hands[card.ability.hand_type].level)])
                }
            },
        }
    end,
    in_pool = function(self, args)
        -- Validates against the hand's visibility state, mirroring vanilla secret hands
        if G.GAME and G.GAME.hands['mxms_f_double_triple'].visible then
            return true
        end
        return false
    end,
    set_card_type_badge = function(self, card, badges)
        badges[#badges + 1] = create_badge(localize('k_mxms_exoplanet'), get_type_colour(card.config.center, card), nil,
        1.2)
    end
}

